...
async def validate_lightning_key(pubkey: str):
    """Valide une clé publique Lightning."""
    if not LightningKeyValidator.is_valid_pubkey(pubkey):
        raise HTTPException(
...
async def validate_lightning_node(node_id: str):
    """Valide un node_id Lightning."""
    if not LightningKeyValidator.is_valid_node_id(node_id):
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
