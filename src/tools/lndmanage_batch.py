...
MAX_WORKERS = 6
OUTPUT_JSON = "lndmanage_results.json"

# --- FONCTION UTILITAIRE ---
def get_lndmanage_metrics(pubkey_from: str, pubkey_to: str, amount_sat: int = 100000):
    try:
        ...
        if result.returncode != 0:
            logging.warning(f"[lndmanage] Erreur pour {pubkey_to}: {result.stderr.strip()}")
            return {"pubkey": pubkey_to, "error": result.stderr.strip()}
        data = json.loads(result.stdout)
        data["pubkey"] = pubkey_to
        return data
    except Exception as e:
        logging.error(f"[lndmanage] Exception pour {pubkey_to}: {e}")
        return {"pubkey": pubkey_to, "error": str(e)}

# --- BATCH PARALLÈLE ---
def batch_lndmanage(pubkey_from: str, pubkeys_to: List[str], amount_sat: int = 100000, max_workers: int = 6):
    results = []
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        future_to_pubkey = {
            executor.submit(get_lndmanage_metrics, pubkey_from, pubkey_to, amount_sat): pubkey_to
        ...
        for future in as_completed(future_to_pubkey):
            ...
            try:
                ...
                logging.info(f"[lndmanage] Résultat pour {pubkey}: {result}")
            except Exception as exc:
                logging.error(f"[lndmanage] Exception pour {pubkey}: {exc}")
...
if __name__ == "__main__":
    ...
    results = batch_lndmanage(PUBKEY_FROM, PUBKEYS_TO, AMOUNT_SAT, MAX_WORKERS)
    # Export JSON
